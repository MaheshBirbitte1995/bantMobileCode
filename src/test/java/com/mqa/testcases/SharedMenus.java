...
public class SharedMenus extends BaseTest  {
	...
	@Test(priority = 1)
	public void Verify_create_SharedMenu() throws InterruptedException, IOException, AWTException {
		msg_inbox.Clickon_Menu_Icon();
		appslogo.SelectProd_Stn();
		
		menus.create_SharedMenu(); //created_menu_on_datalist
		// menus.menu_refresh_btn.click();
		
		Assert.assertEquals("ApimShrdMenu"+menus.random_menu_name.trim(),menus.created_menu_on_datalist.getText().trim(),"Verification failed for created SharedMenus..!!");
	}
